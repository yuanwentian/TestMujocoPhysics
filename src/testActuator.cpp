...
/////TRIAL1: apply force on the ball by set d -> ctrl
//void applyForceOnModel(const mjModel* m, mjData* d)
//{
//    if (d -> time > 5 && d -> time < 8)
//        d -> ctrl[0] = {-4.0};
//if (d->time >= 8)
//    d->ctrl[0] = {4.0};
//}

/////TRIAL2: using controller
//void mycontroller(const mjModel* m, mjData* d)
//{
//   d->ctrl[0] = std::cos(200 * d->time / 10.) * 0.01;
//   d->ctrl[1] = std::sin(200 * d->time / 10.) * 0.01;
//}

/////TRIAL3: test gravity contact
void testForceOnAccCorrect(const mjModel* m, mjData* d)
{
	if (d -> time < 1)
 		 {  
		  d -> ctrl[1] = {.0005}; 	
		 }
	if (d->time > 1 && d->time < 2)
 		 {  
		  d -> ctrl[1] = {-.0005}; 	
		 }	 	 
	 std::cout << "nconmax:" << m->nconmax << std::endl; 
         std::cout << "contact[0]:" << d->contact << std::endl; 
}

/////TRIAL4: test wall contact
void testWallContact(const mjModel* m, mjData* d)
{
    
if (d -> time > 0.2 && d -> time < 0.8)
		{  
			  d -> ctrl[0] = {4};;
		}
if (d -> time > 0.8 && d -> time < 1)
		{  
			  d -> ctrl[0] = {0};;
		}
      std::cout << "nconmax:" << m->nconmax << std::endl; 
